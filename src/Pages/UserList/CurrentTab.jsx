...
//   if (!userId) {
//     return <Alert message="User ID is missing from URL" type="warning" />;
//   }
...
const CurrentBoxTab = ({ userId }) => {
  ...
  if (!boxData?.data) {
    ...
    if (invoiceLoading || boxLoading) return <Spin tip="Loading current invoice and box..." />;
  if (invoiceError) return <Alert message="Error fetching invoice" type="error" description={JSON.stringify(invoiceError)} />;
  if (boxError) return <Alert message="Error fetching box details" type="error" description={JSON.stringify(boxError)} />;
  if (!boxData?.data) return <p>No box details available.</p>;
