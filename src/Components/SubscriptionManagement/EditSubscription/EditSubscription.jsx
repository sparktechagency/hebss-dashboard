...
import { useGetSpecificSubscriptionQuery, useUpdateSubscriptionMutation } from "../../../redux/features/subscriptionApi/subscriptionApi";
import { useEffect } from "react";


const EditSubscription = ({ subscriptionId, onCancel }) => {
    const [updateSubscription] = useUpdateSubscriptionMutation();
    const { data: SpecificSubscription } = useGetSpecificSubscriptionQuery(subscriptionId);

    const [form] = Form.useForm();

    useEffect(() => {
        if (SpecificSubscription?.data) {
            form.setFieldsValue({
                title: SpecificSubscription?.data?.name,
                price: {
                    amount: SpecificSubscription?.data?.price?.amount,
                    currency: SpecificSubscription?.data?.price?.currency
                },
                validity: {
                    type: SpecificSubscription?.data?.validity?.type,
                    value: SpecificSubscription?.data?.validity?.value
                },
                features: SpecificSubscription?.data?.features?.length
                    ? SpecificSubscription?.data?.features.map((feature) => feature)
                    : [""]
            });
        }
    }, [SpecificSubscription, form]);
    ...
    const onFinish = async (values) => {
        ...
        if (subscriptionId) {
            try {
                await updateSubscription({ _id: subscriptionId, data }).unwrap();
            ...
            message.error("Subscription ID is missing");
        }
    };
